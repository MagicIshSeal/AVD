...
%% 2. Inputs (3 structs of your aircraft)
% 2.1 your aircraft struct1 
% aircraft struct 1 (AC1)
AC1_lon_para = struct(...
   'rho', 1.1,          ...                % [kg/m^3] air density
    ...
    'm', 425,           ...                 % [kg] Mass
    'Iyy', 580.63, ...                        % [kg*m^2] Moment of inertia 
    'Ixx', 0, ... 
    'Izz', 0, ... 
    'Ixz', 0  ...
);
...
AC1_lon_coef = struct('C_L', 0.335697, 'C_D', 0.009994, 'C_L_a', 5.471114, 'C_D_a', 0.267205, ...
    'C_m_a', -0.501977, 'C_L_da', -1.53, 'C_m_da', -6.05, 'C_L_hq', 7.83, 'C_m_hq', -35.6, ...
    'C_L_M', 0, 'C_m_M', 0, 'C_L_de', 0.465, 'C_m_de', -2.12);

% 2.2 your aircraft struct2 
% aircraft struct 2 (AC2)
AC2_lon_para = struct(...
   'rho', 1.1,          ...                % [kg/m^3] air density
    'theta_0', 0,                     ...   % [rad] Initial pitch angle
    'U0', 50,                        ...    % [m/s]
    ...
    'c', 0.83,              ...              % [m] Mean aerodynamic chord
    'm', 425,           ...                 % [kg] Mass
    'Iyy', 564.72, ...                        % [kg*m^2] Moment of inertia 
    'Ixx', 0, ... 
    'Izz', 0, ... 
    'Ixz', 0  ...
);
...
% YOUR AIRCRAFT/Aircraft Parameters and Constants Structure 
AC2_lon_coef = struct('C_L',  0.335697, 'C_D',  0.009994, 'C_L_a', 5.471114, 'C_D_a', 0.267205, ...
    'C_m_a', -0.759762, 'C_L_da', -1.53, 'C_m_da', -6.05, 'C_L_hq', 7.83, 'C_m_hq', -35.6, ...
    'C_L_M', 0, 'C_m_M', 0, 'C_L_de', 0.465, 'C_m_de', -2.12);

% 2.3 your aircraft struct3 
% aircraft struct 3 (AC3)
AC3_lon_para = struct(...
    'rho', 1.1,          ...                % [kg/m^3] air density
    'theta_0', 0,                     ...   % [rad] Initial pitch angle
    'U0', 50,                        ...    % [m/s]
    ...
    'c', 0.83,              ...              % [m] Mean aerodynamic chord
    'm', 170,           ...                 % [kg] Mass
    'Iyy', 372.99, ...                        % [kg*m^2] Moment of inertia 
    'Ixx', 0, ... 
    'Izz', 0, ... 
    'Ixz', 0  ...
);
% YOUR AIRCRAFT/Aircraft Parameters and Constants Structure 
AC3_lon_coef = struct('C_L', 0.3368, 'C_D', 0.01, 'C_L_a', 5.55, 'C_D_a', 0.173, ...
    'C_m_a', -0.1, 'C_L_da', -1.53, 'C_m_da', -6.05, 'C_L_hq', 7.83, 'C_m_hq', -35.6, ...
    'C_L_M', 0, 'C_m_M', 0, 'C_L_de', 0.465, 'C_m_de', -2.12);


%% 3. Select the aircraft of interest
% 3.1 Append Names of the selected aircraft
Names = {'Base','Adjusted'}; % Store names for plot legends

% 3.2 Append Parameter structures to compare [para-1, para-2, ..., para-n]
AircraftParams = [AC1_lon_para, ...
                AC2_lon_para]; 

% 3.3 Append Parameter coefficients to compare [coef-1, coef-2, ..., coef-n]
AircraftCoefs = [AC1_lon_coef, ...
                AC2_lon_coef];    
...
for i = 1:length(AircraftParams)  
    ...
    Blon = [0; aeroDerivatives.Zde; aeroDerivatives.Mdestar; 0]/180*pi
    pi();   % Control input: 1 deg elevator deflection
